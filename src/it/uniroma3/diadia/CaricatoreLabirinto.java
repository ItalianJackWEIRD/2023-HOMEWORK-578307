		...
		for(String specifica : separaStringheAlleVirgole(specificheStanze)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) 	{	
				while (scannerDiLinea.hasNext()) {
					...
					presentazione= presentazione.replace("-", " ");
					...
					check(scannerDiLinea.hasNext(),msgTerminazionePrecoce("vi è stato qualche problema nella creazione dell'attrezzo per il mago della stanza "+specifica+"\n"));
					int peso = scannerDiLinea.nextInt();

					AbstractPersonaggio personaggio = new Mago(mago, presentazione, new Attrezzo(attrezzo, peso));
		...
		for(String specifica : separaStringheAlleVirgole(specificheStanze)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) 	{	
				while (scannerDiLinea.hasNext()) {
					...
					String presentazione = scannerDiLinea.next();
					presentazione= presentazione.replace("_", " ");
		...
		for(String specifica : separaStringheAlleVirgole(specificheStanze)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specifica)) 	{	
				while (scannerDiLinea.hasNext()) {
					...
					String presentazione = scannerDiLinea.next();
					presentazione= presentazione.replace("_", " ");
					check(scannerDiLinea.hasNext(),msgTerminazionePrecoce("specifica l'attrezzo del cane\n"));
					String attrezzo = scannerDiLinea.next();
					check(scannerDiLinea.hasNext(),msgTerminazionePrecoce("specifica il peso dell'attrezzo del cane\n"));
					int peso = scannerDiLinea.nextInt();	


					AbstractPersonaggio personaggio = new Cane(cane, presentazione, attrezzo, peso);
					this.nome2stanza.get(nomeStanza).addPersonaggio(personaggio);
				}
			}
		} 
